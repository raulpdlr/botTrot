//+------------------------------------------------------------------+
//|                                           FTSystem-Indicator.mq4 |
//|                                   Forex Trading System Indicator |
//|                                          http://www.ftsystem.org |
//+------------------------------------------------------------------+

#property copyright "www.ftsystem.org"
...
double phaseParam, logParam, Value, series, sValue, sqrtParam, lengthDivider;
//---- temporary int variables
int   s58, s60, s40, s38, s68, Serie2;
//+------------------------------------------------------------------+
//|  initFlagization function                                     |
//+------------------------------------------------------------------+
int init()
  {
   double   lengthParam;
...
//---- name for DataWindow and indicator subwindow label
   IndicatorShortName ("Value(" + Length + "," + Phase + ")");
...
//---- main cycle
   for (int shift = limit; shift >= 0; shift--) {
     //(0-CLOSE, 1-OPEN, 2-HIGH, 3-LOW, 4-MEDIAN)
      if (Serie==0) {series = Close [shift];}
      if (Serie==1) {series = Open [shift];}
      if (Serie==2) {series = High [shift];}
      if (Serie==3) {series = Low [shift];}
      if (Serie==4) {series = (High [shift] + Close [shift])/2;}
      //series = Serie2 [shift];
...
//----			    
			   ...
			   double TempValue, sqrtDivider = sqrtParam / (sqrtParam + 1.0);
...
// ---- end of big cycle                  			   
         if (loopCriteria > 30) {
            ...
            fA8Buffer [shift] = (phaseParam * fC8Buffer [shift] + fC0Buffer [shift] - TempValue) * 
                                 (powerValue * (-2.0) + squareValue + 1) + squareValue * fA8Buffer [shift + 1];  
            TempValue += fA8Buffer [shift]; 
         }
         Value = TempValue;
      }
      if (loopParam <= 30) Value = 0;
      ValueBuffer [shift] = Value;
//---- End of main cycle
   } 
   return;
}
